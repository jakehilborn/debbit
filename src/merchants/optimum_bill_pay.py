...
def web_automation(driver, merchant, amount):
    ...
    logged_in = utils.is_logged_in(driver, timeout=90,
        logged_out_element=(By.ID, 'loginPageUsername'),  # TODO switch to password
        logged_in_element=(By.ID, 'otherAmountInput')
    )

    if not logged_in:
        driver.find_element_by_id('loginPageUsername').send_keys(merchant.usr)  # TODO check for username element before send_keys
